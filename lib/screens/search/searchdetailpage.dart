...
// to show progressbar while loading data in background
  ...
  onTapSelectItemEvent(int index){
    FirebaseEvents().selectItemEvent(
        updateDatas!.products!.packageProducts![index].hotel!.name,
        updateDatas!.products!.packageProducts![index]
            .hotel!.giataId.toString());
  }
                ...
                if (index == itemCount) {
                  ...
                  return
                    GestureDetector(
                      ...
                      onTap: () {
                        onTapSelectItemEvent(index);  /// This event logs which hotel user selecting from list of available hotels in the app.
                        getOverView(
                            updateDatas!.products!.packageProducts![index],
                            updateDatas!.products!.packageProducts![index]
                                .hotel!.giataId
              ...
              if (index == itemCount) {
               ...
               if(memberShipDatas.containsKey(updateDatas!.products!.packageProducts![index].hotel!.giataId.toString())){
                ...
                return GestureDetector(
                    ...
                    onTap: () {
                      onTapSelectItemEvent(index);
                     if(widget.promoCode.isNotEmpty){
                       var promotionName = widget.data.products?.packageProducts![index].topOffer.price?.discountInfo?.discountReason;
                       FirebaseEvents().viewPromotionEvent(
                           "${widget.endDate.difference(widget.startDate).inDays.toString()}",
                           "${widget.data.products?.packageProducts![index].hotel?.name}",
                           "${widget.data.products?.packageProducts![index].hotel?.location?.country.name}-"
                               "${widget.data.products?.packageProducts![index].hotel?.location?.city?.name}",
                           widget.promoCode,
                           promotionName!=null?promotionName:"");
                     }                                               /// This event logs Details of which hotel user selecting from Offers list of hotels in the app.
                      getOverView(
                          updateDatas!.products!.packageProducts![index],
                          updateDatas!.products!.packageProducts![index].hotel!
                              .giataId.toString());
